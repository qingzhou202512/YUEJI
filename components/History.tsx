...
import { Calendar as CalendarIcon, CheckCircle, XCircle, Sparkles, BatteryLow, Battery, Search, ChevronLeft, ChevronRight, X, ArrowLeft } from 'lucide-react';
import { EntryCard } from './history/EntryCard';
  ...
  const renderCalendar = () => {
      ...
      for (let i = 1; i <= totalDays; i++) {
          ...
          days.push(
              <div 
                  key={i} 
                  className={`h-10 w-full flex flex-col items-center justify-center relative rounded-xl border pointer-events-none ${
                      hasRecord ? 'bg-primary-50 text-primary-700 border-primary-100' :
                      'text-ink-700 border-transparent'
                  }`}
              >
                  <span className={`text-sm ${isToday ? 'text-primary-600 font-black' : (hasRecord) ? 'font-bold' : ''}`}>
                      {i}
                  </span>
                  {hasRecord && (
    ...
    <div className="flex flex-col h-full bg-cream overflow-hidden">
      ...
      <div className="flex-none px-6 pt-10 pb-2 bg-cream z-20 flex flex-col gap-4 shadow-sm">
          <div className="flex items-center gap-3">
             ...
             <h2 className="text-2xl font-bold text-ink-900 tracking-tight">足迹</h2>
          ...
          <div className="flex gap-3">
            ...
            <button 
                ...
                className={`flex items-center justify-center w-12 h-full rounded-2xl border transition-all ${
                    ...
                    : 'bg-white text-ink-500 border-transparent shadow-sm active:scale-95'
                }`}
            >
          ...
          <div className={`overflow-hidden transition-all duration-300 ease-in-out ${showCalendar ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="bg-white/60 backdrop-blur-sm rounded-[1.5rem] p-4 border border-white/60 shadow-sm mt-1">
                <div className="flex justify-between items-center mb-4 px-2">
                    <button onClick={handlePrevMonth} className="p-1 rounded-full hover:bg-white text-ink-400 hover:text-ink-900 transition-colors active:scale-90">
                        <ChevronLeft size={20} />
                    </button>
                    ...
                    <button onClick={handleNextMonth} className="p-1 rounded-full hover:bg-white text-ink-400 hover:text-ink-900 transition-colors active:scale-90">
                ...
                <div className="mt-3 text-center">
                    <span className="text-[10px] text-ink-400 font-bold uppercase tracking-wider">记录统计日历</span>
                </div>
            </div>
          </div>
      </div>
